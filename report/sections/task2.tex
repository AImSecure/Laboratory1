...
\section{TASK 2 --- SHALLOW FEED-FORWARD NEURAL NETWORK (FFNN)} \label{sec:shallow-nn}
    ...
    \subsection{Training Dynamics}
        ...
        The 32 and 64-neuron networks achieved the lowest validation loss (\textasciitilde 0.295), with the 64-neuron model early stopping just before 100 epochs. However, all the three models demonstrated similar training dynamics.
    ...
    \subsection{Validation Performance Analysis}
        ...
        Validation accuracy reached about 88\% for 32 and 128 neurons, 89.4\% for 64 neurons (macro F1 \textasciitilde 0.748). 
        So the 64-neuron model outperformed others in overall metrics (Table~\ref{tab:linear-validation}): considering these results and training dynamics, it was selected for detailed class-wise analysis.
        

    \subsection{Activation Function Study and Generalization}
    
        Replacing the linear activation function with ReLU (64 neurons) accelerated convergence and enhanced minority-class recognition (Brute Force F1 \textasciitilde 0.85), as shown in Figure~\ref{fig:relu_64}. 
        ...
        As we can see from the validation report (Table~\ref{tab:relu_64-metrics}), the model improved significantly in identifying Brute Force attacks while maintaining high performance on other classes. 
        So the choice of activation function proved crucial in enhancing model capability of capturing complex patterns or less frequent classes in the dataset.
