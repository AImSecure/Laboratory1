...
\section{TASK 3 --- IMPACT OF SPECIFIC FEATURES (DESTINATION PORT)} \label{sec:impact-dest-port}
    ...
    \subsection{Evaluating Bias via Port Substitution}
        ...
        To quantify this effect, the trained 64-neuron ReLU model was evaluated on a \textbf{modified test set} in which all Brute Force samples had their destination port changed from 80 to 8080. 
        While the model performed well on the original validation set (Brute Force: F1 \textasciitilde 0.85, overall accuracy \textasciitilde 95\%), its performance degraded sharply on the modified test set (Brute Force: F1 \textasciitilde 0.08, overall accuracy \textasciitilde 90\%). 
    ...
    \subsection{Class Balance Considerations}
        ...
        Even after preprocessing, the dataset remains \textbf{imbalanced}, with benign samples far exceeding attack ones and minority classes (Brute Force, PortScan) underrepresented. 
